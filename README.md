...
### Run UI without backend services

If you only need to validate the frontend UI, you can run a local mock mode without LangGraph or Supabase:

1. Set `NEXT_PUBLIC_LOCAL_UI_MODE=true` in `.env`
2. Run the app with `yarn dev`

In this mode, the app uses in-memory mock user/assistant/thread data and bypasses backend auth/proxy checks.
It is intended for local UI testing only (data is not persisted).
