...
export function AssistantProvider({ children }: { children: ReactNode }) {
  ...
  const getAssistants = async (userId: string): Promise<void> => {
    if (isLocalUIModeEnabled()) {
      const localAssistant = getLocalDefaultAssistant(userId);
      setAssistants([localAssistant]);
      setSelectedAssistant(localAssistant);
      return;
    }
  ...
  const createCustomAssistant = async ({
    ...
    if (isLocalUIModeEnabled()) {
      ...
      const createdAssistant = {
        assistant_id: `local-assistant-${Date.now()}`,
        graph_id: "agent",
        ...
        config: {
          configurable: {
            ...
            documents: newAssistant.documents,
          },
        },
  ...
  const editCustomAssistant = async ({
    ...
    if (isLocalUIModeEnabled()) {
      const updatedAssistant = assistants.find(
      ...
      const merged = {
        ...
        config: {
          configurable: {
            ...
            systemPrompt: editedAssistant.systemPrompt,
            documents: editedAssistant.documents,
          },
        },
      } as unknown as Assistant;
      setAssistants((prev) =>
        prev.map((assistant) =>
          assistant.assistant_id === assistantId ? merged : assistant
        )
      );
      if (selectedAssistant?.assistant_id === assistantId) {
        setSelectedAssistant(merged);
      }
      return merged;
    }
