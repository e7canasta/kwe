...
const SystemPromptWhatsThis = (): React.ReactNode => (
  <span className="flex flex-col gap-1 text-sm text-gray-600">
    <p>
      Custom system prompts are passed to the LLM when generating and revising
      contract drafts. They are <i>not</i> used for the generic chat flow or
      for local contract toolbar actions that run directly in the frontend.
    </p>
    <p>
...
export function CreateEditAssistantDialog(
  ...
  }, [getContextDocuments, props.assistant, props.isEditing]);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const contentDocuments =
      documents?.length ? await convertDocuments(documents) : [];
      ...
      <DialogContent className="max-w-xl max-h-[90vh] p-8 bg-white rounded-lg shadow-xl min-w-[70vw] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
        <DialogHeader>
          ...
          <DialogDescription className="mt-2 text-md font-light text-gray-600">
            <TighterText>
              Configure a drafting assistant for a specific workflow, supplier
              context, or contract style.
            </TighterText>
          </DialogDescription>
        </DialogHeader>
        <form
        ...
        >
          ...
          <Input
            ...
            required
            id="name"
            placeholder="Vendor Purchase Drafting"
            value={name}
          ...
          <Input
            ...
            required={false}
            id="description"
            placeholder="Assistant for supplier agreements and purchase orders"
            value={description}
          ...
          <Textarea
            ...
            required={false}
            id="system-prompt"
            placeholder="You are an expert legal operations assistant for supplier contracts..."
            value={systemPrompt}
