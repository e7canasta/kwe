...
import { validateSelectedClauses } from "@/lib/contracts/draft-tools";
import { Clause } from "@opencanvas/shared/types";

interface ValidateRulesPayload {
...
const resolveSelectedClauses = (payload: ValidateRulesPayload): Clause[] => {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const payload = (await req.json()) as ValidateRulesPayload;

    if (process.env.RULES_ENGINE_URL) {
      try {
        const response = await fetch(
          new URL("/validate", process.env.RULES_ENGINE_URL),
          {
            method: "POST",
        ...
        console.warn("Rules engine validate fallback engaged", error);
      }
    }

    const selectedClauses = resolveSelectedClauses(payload);
    const result = validateSelectedClauses(selectedClauses, MOCK_CLAUSES);

    return NextResponse.json(result, { status: 200 });
  } catch (error: any) {
    return NextResponse.json(
      { error: error?.message ?? "Failed to validate clause set" },
      { status: 500 }
    );
  }
}
